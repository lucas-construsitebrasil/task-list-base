...
class MysqliDb
{
    ...
    public function rawQuery($query, $bindParams = null)
    {
        ...
        if ($bindParams == null) {
            ...
            if ($stmt !== false) {
                while ($myrow = $stmt->fetch_array(MYSQLI_ASSOC)) {
                    $res[] = $myrow;
                }
            ...
            $this->reset();
            if ($this->count >= 0 && $status === true) {
                if (strpos($query, 'INSERT INTO') !== false || strpos($query, 'UPDATE') !== false) {
                    $res = true;
                    $status = true;
                }
            }
        ...
        if ($status === false) {
            if (AMBIENTE == 'development' || AMBIENTE == 'provisorio') {
                echo 'erro na query  <br><br>' . $this->getLastQuery();
            } else {
                self::insertLog('ERRO', $this->getLastQuery());
            }

            return false;
        } else {
            self::insertLog('SUCCESS', $this->getLastQuery());
        }
    ...
    public function get($tableName, $numRows = null, $columns = '*')
    {
        ...
        if ($status === false) {
            ...
            if (AMBIENTE == 'development') {
                throw new \Exception('erro na query <br><br>' . $this->_lastQuery);
            }
            self::insertLog('ERROR', $this->_lastQuery);
        } else {
            //select nao grava
            //  self::insertLog('SUCCESS',$this->_lastQuery);
        }
    ...
    private static function insertLog($tipo, $acaoLog)
    {
        if ($tipo == 'ERROR' && $_SESSION['nivel_usuario'] == 7) {
            echo 'PROGRAMADOR: ERRO DE SQL, VERIFIQUE:' . $acaoLog;
        }
        if ($tipo == 'ERROR') { 
            if (AMBIENTE == 'production') {
                \ConstrusiteLogger::insertSql($tipo, $acaoLog, print_r(debug_backtrace(), true));
            }
        }


        if ($acaoLog <> '') {
            if (is_numeric($_SESSION['id_conexao'])) {
                $dir = 'cliente' . '_' . $_SESSION['id_conexao'];
            } else {
                $dir = 'vazio';
            }
            $dir = DIR_APP . 'logs/' . $dir . '/';
            $post = $_POST;
            if (!is_array($_GET)) {
                $_GET = array();
            }
            ...
            $texto = PHP_EOL . "\r\n";
            $texto .= '<logItem>' . chr(13) . chr(10)
                . '<type>' . $tipo . '</type>' . chr(13) . chr(10)
                . '<user>' . $_SESSION['login_usuario'] . '</user>' . chr(13) . chr(10)
                . '<date>' . date('Y-m-d H:i:s') . '</date>' . chr(13) . chr(10)
                . '<page>' . $_SERVER['PHP_SELF'] . '-' . $_SERVER['QUERY_STRING'] . '</page>' . chr(13) . chr(10)
                . '<session>' . 'ID=>' . $_SESSION['id_usuario'] . ',N?VEL=>' . $_SESSION['nivel_usuario']
                . ',NOME=>' . $_SESSION['nome_usuario']
                . '</session>' . chr(13) . chr(10)
                . '<nomeScript>' . $_SERVER['PHP_SELF'] . '</nomeScript>' . chr(13) . chr(10)
                . '<get>' . implode(',', $_GET) . '</get>' . chr(13) . chr(10)
                . '<post>' . implode(',', $post) . '</post>' . chr(13) . chr(10)
                . '<description>' . $acaoLog . ' </description>' . chr(13) . chr(10)
                . '</logItem>' . chr(13) . chr(10);

            \ConstrusiteDefault\PathLogger::setPathLogSql($dir);
            \ConstrusiteLogger::insertSql($tipo, $texto);
        }
    }
    ...
    private function _buildInsert($tableName, $insertData, $operation)
    {
        ...
        if ($status === false) {
            if (AMBIENTE == 'development') {
                throw new \Exception('erro na query ' . $this->_mysqli->error . '<br><br>' . $this->_lastQuery);
            }
            self::insertLog('ERROR', $this->_mysqli->error . ' ---- ' . $this->_lastQuery);
        } else {
            self::insertLog('SUCCESS', $this->_lastQuery);
        }
    ...
    protected function _buildQuery($numRows = null, $tableData = null)
    {
        ...
        if ($this->_tableName == 'ligacoes AS lig1') {
    ...
    /**
     * Abstraction method that will build the ( part of the WHERE statement
     *
     * @return void
     */
